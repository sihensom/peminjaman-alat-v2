<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-black text-foreground tracking-tight">Edit User</h2>
                <p class="text-xs text-muted-foreground font-medium">Ubah data akun: {{ $user->name }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="card-premium overflow-hidden bg-gradient-to-b from-card to-muted/20">
            ...
            <form method="POST" action="{{ route('users.update', $user) }}" class="p-8 space-y-8">
                @csrf @method('PUT')
                <div class="space-y-6">
                    <div class="group">
                        <x-input-label for="name" value="Nama Lengkap" class="text-[10px] font-black uppercase text-muted-foreground mb-2" />
                        <input id="name" name="name" type="text" class="block w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl text-foreground focus:ring-4 focus:ring-indigo-500/20 transition-all font-bold" value="{{ old('name', $user->name) }}" required />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div class="group">
                        <x-input-label for="email" value="Email" class="text-[10px] font-black uppercase text-muted-foreground mb-2" />
                        <input id="email" name="email" type="email" class="block w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl text-foreground focus:ring-4 focus:ring-indigo-500/20 transition-all font-bold" value="{{ old('email', $user->email) }}" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="group">
                            <x-input-label for="password" value="Password Baru (Opsional)" class="text-[10px] font-black uppercase text-muted-foreground mb-2" />
                            <div class="relative">
                                <input id="password" name="password" type="password" class="block w-full px-4 py-3 pr-12 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl text-foreground focus:ring-4 focus:ring-indigo-500/20 transition-all font-bold" />
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                <input id="password_confirmation" name="password_confirmation" type="password" class="block w-full px-4 py-3 pr-12 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl text-foreground focus:ring-4 focus:ring-indigo-500/20 transition-all font-bold" />
                ...
                <div class="flex items-center justify-end gap-6 pt-10 mt-10">
                    ...
                    <button type="submit" class="px-8 py-3 bg-indigo-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:-translate-y-0.5 active:translate-y-0 transition-all">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
